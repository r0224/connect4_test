...
class connect4_system:
    ...
    def one_move(self, choice:int, player="player_1"):
        assert (0 <= choice and choice <= 6), "おける列は0~6で指定してください。"
        assert (player == "player_1"  or  player == "player_2"), "おける列は0~6で指定してください。"
        if self.board_data[0, choice] != 0:
            # その列は埋まっていて石をおけない。
            return False
        else:
...
def vs_mode(conect_board:connect4_system, palyer_strategy_dict, start_player="player_1", display=True):
    # cpu vs cpu
        ...
        while True:
            ...
            conect_board.one_move(player_choice, player=turn_player)
...
if __name__ == "__main__":
    ...
    # palyer_strategy_dict = {"player_1":[MCTS.mcts, {"turn_player": "player_1", "playout_num":1000}], "player_2": [strategy_random, {}]}
    # palyer_strategy_dict = {"player_2":[MCTS.mcts, {"turn_player": "player_2", "playout_num":3000, "ucb_const":2}], "player_1": [strategy_random, {}]}

    # palyer_strategy_dict = {"player_1":[MCTS.mcts, {"turn_player": "player_2", "playout_num":2000, "ucb_const":1.5}], "player_2": None}

    # palyer_strategy_dict = {"player_1":[MCTS.mcts, {"turn_player": "player_1", "playout_num":2000, "ucb_const":2}], "player_2": [strategy_random, {}]}

    palyer_strategy_dict = {"player_1":[MCTS.mcts, {"turn_player": "player_1", "playout_num":2000, "ucb_const":1.5}], "player_2": None}
    ...
    for x in range(1):
        ...
        y = vs_mode(conect_board, palyer_strategy_dict)
